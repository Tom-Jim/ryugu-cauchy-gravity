...
mod tests {
    ...
    fn cube_volume_is_exact() {
        let v = body_volume(&unit_cube());
        assert!((v - 8.0).abs() < 1e-12, "volume {v}");
    }

    #[test]
    fn radial_g_limit() {
        assert!((radial_g(0.0) - 1.0 / 3.0).abs() < 1e-15);
        // Series and direct branches must agree at the t = 0.1 cutoff. Below it
        // the direct form is the one that is wrong (`t − atan t` cancels): at
        // t = 1e-3 it is already off by ~1e-10 relative, which is the whole
        // reason the series branch exists.
        let t = 0.1f64;
        let direct = (t - t.atan()) / (t * t * t);
        let series = radial_g_series(t);
        assert!(
            (series - direct).abs() < 1e-13 * direct.abs(),
            "t={t} series={series} direct={direct}"
        );
        // And the identity 3g + t g′ = 1/(1 + t²) that defines g at all.
        for t in [1e-6f64, 1e-3, 0.05, 0.3, 1.0, 4.0, 50.0] {
            let h = t * 1e-6;
            let deriv = (radial_g(t + h) - radial_g(t - h)) / (2.0 * h);
            let lhs = 3.0 * radial_g(t) + t * deriv;
            let rhs = 1.0 / (1.0 + t * t);
            assert!((lhs - rhs).abs() < 1e-9, "t={t} lhs={lhs} rhs={rhs}");
        }
    }

    #[test]
    fn sphere_matches_closed_form() {
        for sigma in [0.05f64, 0.28, 2.0] {
            let mesh = uv_sphere(0.5, 96);
            let got = kernel_volume(&mesh, [0.0, 0.0, 0.0], sigma);
            let want = ball_kernel_volume(0.5, sigma);
            let rel = (got - want).abs() / want;
            assert!(rel < 5e-4, "sigma={sigma} got={got} want={want} rel={rel}");
        }
    }
}
