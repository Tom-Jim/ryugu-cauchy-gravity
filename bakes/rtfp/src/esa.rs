...
//! is checked against.
//!
//! The library is linked behind the `esa` cargo feature because it drags in a
//! vendored C++ checkout (ESA polyhedral-gravity plus yaml-cpp, spdlog, tetgen
//! and thrust). Neither the RT-FP split nor the Carlson jump-surface solver
...
//!
//! ```text
//! bun run esa:configure && bun run bakes:build
//! ```
//!
...
pub const NOT_LINKED: &str = "the ESA reference library is not linked: rebuild with \
     `--features esa` after `bun run esa:configure` and `bun run bakes:build`";
