...
The former 16 m comparison table was generated by the removed tetra-centre
density-jump Carlson approximation and is therefore not evidence for the
rewritten backend. New accuracy and timing numbers must be generated from the
same continuous density, observation points, direction rule and reference
tensor. Until that validation is recorded, this repository does not publish a
numerical error or speedup claim for the rewritten Carlson path.
