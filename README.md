...
The viewer is desktop-only for now: Android and iOS layouts are not supported
yet, and the page displays a compatibility warning on phones, tablets, and
narrow viewports. Desktop Chrome and Firefox are verified with WebGPU.
...
environment. The Pages bundle intentionally uses the development Rust profile,
which preserves the WebGPU handle semantics required by the current Bevy/wgpu
stack on WASM. Neither `pkg/` nor `dist/` is committed to the repository.
