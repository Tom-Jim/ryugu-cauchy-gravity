...
pub fn explode_mesh_for_flat_faces(src: &Mesh) -> Option<(Mesh, usize)> {
    ...
    let uvs = match src.attribute(Mesh::ATTRIBUTE_UV_0) {
        Some(VertexAttributeValues::Float32x2(uv)) => Some(uv.as_slice()),
        _ => None,
    };
    let vertex_colors = match src.attribute(Mesh::ATTRIBUTE_COLOR) {
    ...
    let mut new_uv = uvs.map(|_| Vec::with_capacity(n_corners));
    let mut new_colors = vertex_colors.map(|_| Vec::with_capacity(n_corners));
    ...
    for face in 0..(n_corners / 3) {
        ...
        for vi in [i0, i1, i2] {
            ...
            if let (Some(uvs), Some(new_uv)) = (uvs, new_uv.as_mut()) {
                new_uv.push(uvs.get(vi).copied().unwrap_or([0.0, 0.0]));
            }
            if let (Some(colors), Some(new_colors)) = (vertex_colors, new_colors.as_mut()) {
                new_colors.push(colors.get(vi).copied().unwrap_or([1.0; 4]));
            }
        }
    }
...
pub fn ensure_base_colors(mesh: &mut Mesh) -> bool {
    let n = mesh.count_vertices();
    if n == 0 {
        return false;
    }
    if !matches!(
        ...
        mesh.insert_attribute(Mesh::ATTRIBUTE_COLOR, vec![[1.0; 4]; n]);
    }
    true
}
...
pub struct BakePaint {
    ...
    pub rng: u64,
    /// When true, next paint pass restores the original GLB vertex colours.
    pub reset_to_base: bool,
    /// Original per-vertex colours captured before any scalar overlay.
